...
class StackObj:
    ...
    @next.setter
    def next(self, next: 'Optional[StackObj]'):
        self._next = next
...
class Stack(StackInterface):
    ...
    def pop_back(self) -> Optional[StackObj]:
        ...
        self._length -= 1
        if self._top.next is None:  # 1 node in the stack
            return self._pop_from_one_node_stack()
        return self._pop_back_in_long_stack()  # 2+ nodes in the stack
    ...
    def _push_back_in_filled_stack(self, obj: StackObj):
        current_node = self._top  # not empty stack
    ...
    def _pop_back_in_long_stack(self) -> StackObj:
